...
export default function App() {
  ...
  async function handleDelete(id:string) {
    try{
      await api.delete("/customer", {params: {
        id:id,
      }
    })
      ...
      <main className="my-10 w-full md:max-w-2xl">
        ...
        <form className="flex flex-col my-6" onSubmit={handleSubmit}>
          ...
          <input 
            type="submit"
            value="Cadastrar"
            className="mt-6 cursor-pointer w-full p-2 bg-green-800 font-medium text-white rounded" 
          />
        </form>
        ...
        <section className="flex-col flex gap-4">
          ...
          {customers.map((customer)=>(
            <article
              key={customer.id}
              className="w-full bg-gray-300 rounded p-2 relative hover:scale-105 duration-200"
            >
